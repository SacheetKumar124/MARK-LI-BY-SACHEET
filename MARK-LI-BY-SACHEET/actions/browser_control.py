...
class _BrowserSession:
    ...
    def start(self):
        ...
        if self._startup_error:
            raise RuntimeError(
                f"Playwright browser driver could not start for '{self.browser_name}': "
    ...
    async def screenshot(self, path: str = None) -> str:
        if _OS == "Linux":
            try:
                ...
                save_path = path or str(Path.home() / "Pictures" / "jarvis_browser_screenshot.jpg")
                save_file = Path(save_path).expanduser().resolve()
                if Path.home() not in save_file.parents:
                    save_file = Path.home() / "Pictures" / "jarvis_browser_screenshot.jpg"
                save_file.parent.mkdir(parents=True, exist_ok=True)
                save_file.write_bytes(data)
                return f"Desktop browser screenshot saved: {save_file}"
            except Exception as e:
                print(f"[Browser] Native Linux screenshot fallback failed: {e}")
        page = await self._get_page()
        try:
...
def browser_control(
    ...
    if action in {"switch", "new_tab", "close_tab"}:
        result = (
            "Browser tab automation is disabled. Use live_chat for messaging or "
            "linux_desktop/computer_control for explicit desktop actions."
        )
        _log(player, result)
        return result

    if action == "switch":
        visible_target = str(params.get("description", "")).strip()
        if visible_target:
            from actions.computer_control import computer_control
            result = computer_control({
                "action": "screen_click",
                "description": f"{visible_target} tab in the {browser or 'current'} browser tab bar",
            }, player=player)
            _log(player, result)
            return result
    ...
    if action in ("go_to", "search", "new_tab"):
        visible_target = str(params.get("description", "")).strip()
        if visible_target and "tab" in visible_target.lower():
            from actions.computer_control import computer_control
            result = computer_control({
                "action": "screen_click",
                "description": f"{visible_target} in the {browser or 'current'} browser tab bar",
            }, player=player)
            _log(player, result)
            return result
