...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "For one user request, call this tool EXACTLY ONCE. Wait for the image "
            "analysis before doing anything else; never repeat the capture to get more detail. "
    ...
    },
    {
        ...
        "description": (
            ...
            "Opening, switching, and closing browser tabs are disabled. "
            ...
            "automation browser. "
            "For a tab visibly open in the user's desktop browser, do NOT use switch or create an "
            "automation session; use computer_control screen_click and preserve the browser named "
            "by the user (especially Firefox). "
        ...
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":      {"type": "STRING", "description": "go_to | search | click | type | scroll | fill_form | smart_click | smart_type | get_text | get_url | press | screenshot | back | forward | reload | list_browsers | close | close_all"},
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        self._enhanced_live = False  # stable v1beta audio config; preview features are opt-in after a successful session
    ...
    def set_speaking(self, value: bool):
        ...
        try:
            ...
            # Qt objects can be destroyed while asyncio audio tasks unwind.
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "open_app":
                ...
                if self._vision_busy or self._pending_vision or (_now - self._vision_last_time) < _cooldown:
                    _wait = max(0, _cooldown - (_now - self._vision_last_time))
                    print(f"[Vision] ⏳ Single-capture guard active ({_wait:.1f}s remaining) — ignoring duplicate call")
                    result = "One screen capture is already being analyzed. Do not capture the screen again; continue using the first image."
                else:
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                await self.session.send_client_content(
                                    turns={"parts": [
                                        {"inline_data": {"mime_type": mime_t, "data": img_b}},
                                        {"text": (
                                            f"{question}\n\n"
                                            "Perform one deep analysis of this image. Read all legible on-screen "
                                            "text exactly, identify the active application and visible tabs, "
                                            "describe the UI layout, colors, objects, and overall appearance, "
                                            "and explain what is happening. Clearly separate visible facts from "
                                            "uncertainty. Use this same image for the complete answer and do "
                                            "not request another screen capture. Never claim the screen is black "
                                            "unless the pixels are actually black."
                                        )},
                                    ]},
                                    turn_complete=True,
                                )
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            print(f"[JARVIS] ⚠️ Audio output unavailable: {exc}")
            self.set_speaking(False)
            return
