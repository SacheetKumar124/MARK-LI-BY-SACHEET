...
class EdgeTTSEngine:
    ...
    async def _synth(self, text: str) -> bytes:
        try:
            ...
            # The engine packages are opt-in and not in requirements.txt, so the
            # failure has to say what to install rather than surfacing a bare
            # ModuleNotFoundError from inside a background thread.
            raise RuntimeError(
                "EdgeTTS needs the 'edge-tts' package: pip install edge-tts"
            ) from exc
