...
def _pip_extra_flags() -> list[str]:
    """Extra pip flags where the interpreter is marked externally managed.

    Kali, Debian and Ubuntu ship an EXTERNALLY-MANAGED marker (PEP 668): a plain
    ``pip install`` outside a virtualenv is refused, so in-app dependency
    installation would fail silently on the very distro this runs on.  ``--user``
    keeps every file in the owner's home directory; nothing is written to /usr.
    """
