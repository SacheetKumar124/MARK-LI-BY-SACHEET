...
def _transport_works(idx: int, kind: str, api_key) -> bool:
    ...
    try:
        ...
        try:
            from core.audio_stream import quiet_stderr as _quiet
        except Exception:                      # pragma: no cover - defensive only
            from contextlib import nullcontext as _quiet
        ...
        if kind == "output":
            ...
            #
            # Opened under quiet_stderr: PortAudio prints every refused ALSA rate
            # from C, unbuffered and un-catchable, and this loop asks every
            # listed device — including raw hw: PCMs that cannot resample — so a
            # single picker refresh used to fill the console with ALSA internals.
            with _quiet():
...
def _usable(idx: int, kind: str) -> bool:
    ...
    try:
        ...
        try:
            ...
            from contextlib import nullcontext as _quiet
        rate = _RATES.get(kind, 16000)
        # A refusal is the expected answer here — that is what this function is
        # asking — so PortAudio's C-level narration of it is noise, not news.
        # Without this, listing the picker emitted a dozen "Expression
        # 'paInvalidSampleRate' failed in ... pa_linux_alsa.c" lines for devices
        # the list then correctly excluded.
        with _quiet():
            if kind == "input":
                st = sd.InputStream(samplerate=rate, channels=1, dtype="int16",
                                    blocksize=1024, device=idx,
