...
try:
    # Use the session-aware bridge, which routes to ydotool on Wayland and to
    # real pyautogui on X11.  The guard is deliberately broad: pyautogui raises
    # Xlib's DisplayConnectionError (NOT ImportError) when it cannot reach the X
    # server, which is the normal situation on a Kali GNOME Wayland session.
    # Catching only ImportError let that exception escape and killed this
    # module's import, silently removing the whole desktop_control action.
    from core import desktop_input as pyautogui
    _PYAUTOGUI = True
except Exception:
    pyautogui = None
    _PYAUTOGUI = False
