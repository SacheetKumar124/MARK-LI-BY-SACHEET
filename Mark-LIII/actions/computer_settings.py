...
from core import kali_compat as kc  # wpctl / logind / portal, session-aware
pyautogui = di  # session-aware: ydotool on Wayland, real pyautogui on X11
_PYAUTOGUI = True
...
def volume_up():
    if _OS == "Windows":
        ...
        kc.volume_step(10)  # wpctl on Kali/PipeWire, pactl or pamixer elsewhere
...
def volume_get() -> int | None:
    ...
    try:
        ...
        return kc.volume_get()  # wpctl -> pactl -> pamixer
    except Exception:
        return None
...
def brightness_get() -> int | None:
    ...
    try:
        if _OS == "Windows":
            ...
            return max(0, min(100, int(r.stdout.strip())))
        if _OS == "Linux":
            return kc.brightness_get()  # sysfs read, no root needed
    except Exception:
        pass
    return None
...
def brightness_up():
    if _OS == "Darwin":
        ...
        # logind (polkit, no root) -> brightnessctl -> light.  The old X11
        # `xrandr --brightness` fallback was removed: it never worked on a
        # Wayland session, and it ran through a shell.
        kc.brightness_step(10)
    else:
        try:
            subprocess.run(
                ["powershell", "-Command",
...
def maximize_window():
    if _OS == "Darwin":
        ...
        kc.window_action("maximize")  # ydotool keybinding on Wayland, wmctrl on X11
...
def take_screenshot():
    if _OS == "Windows":
        ...
        # Cascade: xdg-desktop-portal -> GNOME Shell dbus -> grim -> legacy X11.
        # scrot/gnome-screenshot/import are all absent or blind on Kali GNOME
        # Wayland, and `import -window root` cannot see a Wayland frame buffer.
        ok, detail = kc.screenshot()
        if not ok:
            print(f"[Settings] screenshot failed: {detail}")
        return
