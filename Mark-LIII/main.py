    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "value": {"type": "STRING", "description": "Concise value in English (e.g. Sacheet, pizza, older sister)"},
            },
...
class JarvisLive:
    ...
    def speak(self, text: str):
        # Mirror what is spoken into the audit trail and the phone session, so
        # the phone can show (and re-speak) it and "why did you say that?" has
        # an answer later. Never allowed to break speech itself.
        try:
            from core import activity_log, phone_bridge
            activity_log.record("speech", "spoke", detail=str(text)[:400],
                                why="response to the user")
            phone_bridge.note("jarvis", str(text)[:400])
        except Exception:
            pass
        if not self._loop or not self.session:
            return
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if name == "save_memory":
            ...
            # get_running_loop() rather than get_event_loop(): inside a coroutine
            # they are equivalent, but get_event_loop() is deprecated since 3.12
            # and on Python 3.14 raises outright when no loop is running. This
            # form also fails loudly if the call is ever moved out of a coroutine
            # (into a thread, say) instead of quietly returning the wrong loop.
    ...
    async def _send_realtime(self):
        ...
        try:
            # core.audio_stream owns the open. It never selects a raw hardware
            # PCM (the device whose fixed 44100 Hz clock produced
            # "paInvalidSampleRate" straight from PortAudio's C layer, where no
            # try/except could reach it), and if a device will not accept the
            # required rate it opens at one it will and converts in software.
            # So the blocks arriving below are always 16 kHz mono.
            def _on_block(pcm: bytes):
                # Re-uses the existing callback unchanged, which keeps the
                # wake-word gate, the mute gate and the HUD level in one place.
                # frames/time_info/status are unused by it, so byte length and
                # None are the honest arguments here.
                callback(
                    np.frombuffer(pcm, dtype=np.int16),
                    len(pcm) // np.dtype(np.int16).itemsize,
                    None,
                    None,
                )
            ...
            try:
                _mic_stream = audio_stream.open_input(
                    SEND_SAMPLE_RATE,
                    _on_block,
                    channels=CHANNELS,
                    preferred_name=_mic_name or "",
                )
            except Exception as _e:
                # Nothing opened at all — every candidate and every rate was
                # refused. Say that in one plain sentence and stay alive: a
                # missing microphone must not cost the user the whole assistant,
                # and it must not arrive as a stack trace.
                print(f"[JARVIS] ❌ Mic unavailable: {_e}")
                self.ui.write_log(
                    "SYS: No working microphone found — Jarvis cannot hear you."
                )
                while True:
                    await asyncio.sleep(1.0)

            print(f"[JARVIS] 🎤 Mic stream open ({_mic_stream.label} @ {_mic_stream.rate} Hz)")
            try:
                while True:
    ...
    async def _receive_audio(self):
        ...
        try:
            ...
            # ── Server-initiated session rotation ────────────────────────────
            # A Live session has a maximum duration. When it is reached the
            # server sends a GoAway signal and then closes with 1008
            # "...failed to close the connection after receiving a GoAway
            # signal once the session duration...". That is the protocol
            # working as designed, not a fault.
            #
            # Treating it as a fault had two costs, both observed in one run:
            # a traceback that looks like a crash, and — because the failure
            # cancelled the enclosing TaskGroup, which cancelled the audio
            # tasks mid-write — a segfault inside PortAudio during teardown.
            # The reconnect signal below is the correct route: no backoff, and
            # the resumption handle is kept on purpose, because the handle is
            # exactly what a GoAway exists to make possible.
            if (
                "goaway" in _err
                or "go away" in _err
                or "1008" in _err
                or "session durat" in _err
                or "policy violation" in _err
                or ("closing" in _err and "connection" in _err)
            ):
                print("[JARVIS] 🔄 Session rotated by the server — resuming")
                self.ui.write_log("SYS: Session rotated by the server — resuming…")
    ...
    async def _play_audio(self):
        ...
        try:
            stream = audio_stream.open_output(
                RECEIVE_SAMPLE_RATE,
                channels=CHANNELS,
                preferred_name=_spk_name or "",
            )
        except Exception as _e:
            # No speaker at all. One plain sentence, then stay alive rather than
            # taking the session down with a traceback.
            print(f"[JARVIS] ❌ Speaker unavailable: {_e}")
            self.ui.write_log("SYS: No working speaker found — Jarvis cannot speak.")
            while True:
                await asyncio.sleep(1.0)
        ...
        try:
            while True:
                ...
                # Deliberately synchronous and non-blocking: this only appends to
                # a bounded buffer that PortAudio's own thread drains. The old
                # code awaited a blocking write on a thread, and a cancellation
                # landing inside that write is precisely what let ALSA segfault
                # during teardown. Nothing here can be interrupted mid-device.
                stream.write(bytes(batch))
        except Exception as e:
            print(f"[JARVIS] ❌ Play: {e}")
            raise
        finally:
            self.set_speaking(False)
            stream.close()   # idempotent, and cannot raise on a vanished device
    ...
    # ── Assistant brain (proactive senses → attention → speech) ────────────────

    async def _run_assistant_brain(self) -> None:
        """Proactive loop: senses notice, attention decides, Jarvis phrases.

        Mirrors the existing monitor loops. The brain decides *whether*
        something is worth saying (scored, budgeted, deduplicated); this loop
        only decides *when* it is polite to say it — never over the user and
        never mid-sentence.
        """
        await asyncio.sleep(90)              # let the session settle after startup
        while True:
            try:
                await asyncio.sleep(max(15, self._brain.tick_seconds))
                result = await asyncio.to_thread(self._brain.tick)
            except Exception as exc:
                print(f"[Brain] tick failed: {exc}")
                await asyncio.sleep(60)
                continue
            for message in result.get("interrupts", []):
                ...
                try:
                    ...
                    print(f"[Brain] could not deliver alert: {exc}")
    ...
    # ── main loop ───────────────────────────────────────────────────────────        async def run(self):
            # Plugins speak through this loop (`plugin_say` -> run_coroutine_threadsafe
            # from their worker threads), so it must be the loop that is genuinely
            # running. get_running_loop() guarantees that; get_event_loop() only
            # happened to return it, and is deprecated from 3.12 onward.
        ...
        # Prove both directions open at the exact rates the model requires before
        # the session needs them. A device that cannot is then one startup line
        # instead of a wall of ALSA internals mid-conversation. Both streams are
        # closed before returning.
        try:
            _pf = audio_stream.preflight(SEND_SAMPLE_RATE, RECEIVE_SAMPLE_RATE)
            if _pf.get("ok"):
                print(
                    f"[Audio] preflight ok — mic '{(_pf['input'] or {}).get('device')}' "
                    f"and speaker '{(_pf['output'] or {}).get('device')}' "
                    f"at {SEND_SAMPLE_RATE}/{RECEIVE_SAMPLE_RATE} Hz"
                )
            else:
                _bad = [
                    f"{_k}: {(_pf.get(_k) or {}).get('error', 'unavailable')}"
                    for _k in ("input", "output")
                    if not (_pf.get(_k) or {}).get("ok")
                ]
                print(f"[Audio] preflight failed — {'; '.join(_bad)}")
        except Exception as _e:
            print(f"[Audio] preflight skipped: {_e}")
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    tg.create_task(self._run_assistant_brain())
