...
def _ensure_network_access(port: int) -> None:
    ...
    """
    import sys, subprocess, os, tempfile, threading, shutil
    ...
    # ── Linux ─────────────────────────────────────────────────────────────────
    # Elevation policy, deliberately narrow.
    #
    # `pkexec` is NOT used here. It pops a graphical password dialog, and an
    # assistant that asks for root every time the dashboard starts is asking the
    # user to trust it with more than it needs. Opening a firewall port is a
    # security change that should be deliberate, so only the non-interactive
    # `sudo -n` is attempted: it either already works (no prompt) or fails
    # quietly, and the exact command is printed for the user to run.
    def _privileged(cmd: list[str]) -> bool:
        try:
            r = subprocess.run(["sudo", "-n"] + cmd, capture_output=True, timeout=30)
            return r.returncode == 0
        except Exception:
            return False

    # Nothing to open when no firewall manager is installed. Checked first
    # because probing a missing binary three times in a row to reach the same
    # conclusion is just noise — observed on a plain Kali install, where the
    # box has iptables but neither ufw nor firewalld.
    if not any(shutil.which(tool) for tool in ("ufw", "firewall-cmd", "iptables")):
        return

    # One table instead of three near-identical blocks, so "is this manager
    # even installed", "is it active" and "what do I print if it fails" are
    # answered in one place.
    #
    #   binary | probe | open | follow-up | manual hint | "active" marker
    managers: tuple[tuple[str, list, list, Optional[list], str, str], ...] = (
        ("ufw", ["ufw", "status"], ["ufw", "allow", f"{port}/tcp"], None,
         f"sudo ufw allow {port}/tcp", "active"),
        ...
        ("iptables", ["iptables", "-L", "INPUT", "-n"],
         ...
         None, f"sudo iptables -A INPUT -p tcp --dport {port} -j ACCEPT", ""),
    )

    for binary, probe, open_cmd, follow_up, manual, marker in managers:
        if not shutil.which(binary):
            continue        # not installed: nothing to open, and no noise about it
        try:
            probe_run = subprocess.run(probe, capture_output=True, text=True, timeout=5)
        except (FileNotFoundError, OSError):
            continue

        if marker:
            if marker not in (probe_run.stdout or "").lower():
                continue    # installed but not active — nothing to open
        elif probe_run.returncode != 0:
            # iptables cannot even list its rules without root (exit 4 on this
            # Kali install), so no rules are in the way. Deliberately does not
            # escalate just to find that out.
            return

        opened = _privileged(open_cmd) and (follow_up is None or _privileged(follow_up))
        if opened:
            print(f"[Dashboard] {binary}: port {port} opened.")
        else:
            print(f"[Dashboard] {binary} could not open port {port} without a prompt."
                  f"\n[Dashboard] Run manually:  {manual}")
        return
...
def _local_ip() -> str:
    ...
    for probe in ("8.8.8.8", "1.1.1.1", "9.9.9.9"):
        try:
...
class DashboardServer:
    ...
    def _build_app(self) -> "FastAPI":
        ...
        # Assistant endpoints (attention state, digest, audit trail, phone
        # handoffs) live in their own module so this file stays as it was.
        # The phone page is served without auth because it is only a shell;
        # every data endpoint behind it reuses the token check above.
        try:
            from dashboard.assistant_api import build_router as _build_assistant_router
            _assistant_router = _build_assistant_router(_auth, PORT)
            if _assistant_router is not None:
                app.include_router(_assistant_router)
        except Exception as _assistant_exc:                   # noqa: BLE001
            print(f"[Dashboard] assistant API unavailable: {_assistant_exc}")
...
<p>Connecting to JARVIS…</p>
    ...
    async def _serve_alias(self) -> None:
        ...
        # get_running_loop() is the correct call inside a coroutine (3.12+
        # deprecates get_event_loop() and 3.14 can raise from it).
